...
# Pull código mais recente (se for repo git)
if [[ -d ".git" ]]; then
    echo "[git] Atualizando código..."
    git pull --ff-only origin main
fi

# Subir serviços (sem dashboard, sem n8n)
echo "[docker] Subindo serviços: redis, api, worker, beat, caddy"
docker compose up -d ${BUILD_FLAG} --remove-orphans redis api worker beat caddy
